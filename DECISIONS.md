...
### Keyframe pairs only (for now)
Each animated property exports as a single start→end transition using the first and last selected keyframe. Multi-keyframe sequences (e.g. scale up then scale down) are a planned future addition but not in scope yet.
...
- **Multi-keyframe sequences** — export a property's full selected keyframe sequence, not just first/last pair. E.g. a bounce: 0 → 100 → 80 → 90. Data shape TBD once we have examples.
